...
public class NavigationManager {
	...
	public void addMovement(Player player, Event event, int x, int y) {
		ArrayList<Point> path = findPath(new Point(player.getFootX() / GRID_SIZE, 
				player.getFootY() / GRID_SIZE), 
				new Point ((x + GRID_SIZE /2) / GRID_SIZE, (y + GRID_SIZE /2) / GRID_SIZE));
		...
		for (int i = 0; i < mapWidth / GRID_SIZE; i++) {
			...
			for (int j = 0; j < mapHeight / GRID_SIZE; j++) {
				if (entitys.getEntity(i * GRID_SIZE, j * GRID_SIZE) != null 
						|| i < 3 || j < 5 || i > mapWidth / GRID_SIZE - 3 || j > mapHeight / GRID_SIZE - 3) {
					row.add(false);
				} else {
					row.add(true);
				}
	...
	public void rendermesh(Graphics g) {
		...
		if (!this.playerPathes.isEmpty()) {
			for (int i = 0; i < playerPathes.get(0).size(); i++) {
				g.fillRect(playerPathes.get(0).get(i).getX() * GRID_SIZE, playerPathes.get(0).get(i).getY() * GRID_SIZE, 5, 5);
			}
		}
	}
	...
	private Point getRealEnd(Point end) {
		...
		BasicEntity newEntity = entitys.getEntity(end.getX() * GRID_SIZE, end.getY() * GRID_SIZE);
		int leftDistance = Math.abs(newEntity.getX() - end.getX() * GRID_SIZE);
		int rightDistance = Math.abs(newEntity.getX() + newEntity.getWidth() - end.getX() * GRID_SIZE);
		int downDistance = Math.abs(newEntity.getY() + newEntity.getHeight() - end.getY() * GRID_SIZE);
		int upDistance = Math.abs(newEntity.getY() + newEntity.getHeight() / 2 - end.getY() * GRID_SIZE);
		
		if (leftDistance <= rightDistance && leftDistance <= downDistance && leftDistance <= upDistance) {
			realEnd = new Point((newEntity.getX() - GRID_SIZE / 2) / GRID_SIZE, end.getY());
		} else if (rightDistance < leftDistance && rightDistance < downDistance && rightDistance < upDistance) {
			realEnd = new Point((newEntity.getX() + GRID_SIZE + newEntity.getWidth()) / GRID_SIZE, 
					end.getY());
		} else if (downDistance < rightDistance && downDistance < leftDistance && downDistance < upDistance) {
			realEnd = new Point(end.getX(), (newEntity.getY() + GRID_SIZE + newEntity.getHeight()) / GRID_SIZE);
		} else if (upDistance < leftDistance && upDistance < downDistance && upDistance < rightDistance) {
			realEnd = new Point(end.getX(), (newEntity.getY() + newEntity.getHeight() / 2) / GRID_SIZE);
		}
	...
	private ArrayList<Point> createPath(int[][] area, Point start, Point end) {
		...
		lowestPoint = new Point(neighbours.get(0).getX(), neighbours.get(0).getY());
		path.add(end);
		while(!(start.getX() == end.getX() && start.getY() == end.getY()) && lowestValue != 1) {
			neighbours = getNeighbours(lowestPoint);
			for (int i = 0; i < neighbours.size(); i++) {
				if (area[neighbours.get(i).getX()][neighbours.get(i).getY()] < lowestValue) {
					lowestPoint = new Point(neighbours.get(i).getX(), neighbours.get(i).getY());
					lowestValue = area[neighbours.get(i).getX()][neighbours.get(i).getY()];
				}
			}
	...
	private void createArea(int[][] area, Point start, Point end, int deathTime) {
		...
		for (int i = 0; i < neighbours.size(); i++) {
			if (navMesh.get(neighbours.get(i).getX()).get(neighbours.get(i).getY())) {
				if (area[neighbours.get(i).getX()][neighbours.get(i).getY()] > area[start.getX()][start.getY()] + 1) {
					area[neighbours.get(i).getX()][neighbours.get(i).getY()] = area[start.getX()][start.getY()] + 1;
					if (neighbours.get(i).getX() == end.getX() && neighbours.get(i).getY() == end.getY()) {
						return;
					} else {
						createArea(area, new Point(neighbours.get(i).getX(), neighbours.get(i).getY()), end, deathTime - 1);
					}
				}
			}
		}
	...
	private ArrayList<Point> getNeighbours(Point point) {
		...
		if (point.getX() - 1 > 0 && point.getY() + 1 < navMesh.get(0).size() - 1) {
			newArea.add(new Point(point.getX() - 1, point.getY() + 1));
		}
		if (point.getX() + 1 < navMesh.size() - 1) {
			newArea.add(new Point(point.getX() + 1, point.getY()));
		...
		if (point.getX() + 1 < navMesh.size() - 1 && point.getY() + 1 < navMesh.get(0).size() - 1) {
			newArea.add(new Point(point.getX() + 1, point.getY() + 1));
		}
